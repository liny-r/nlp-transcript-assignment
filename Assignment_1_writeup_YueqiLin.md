...
The prompt used double-brace escaping for the JSON template to avoid Python f-string collisions — a non-obvious gotcha documented in §6.8.
...
From the per-call extractions, 18 features were built for each (ticker, quarter) row across four engineering steps (§5–6e):
...
2. **Full-feature LogReg (§9a):** LogReg trained on all 18 engineered features. Hyperparameters (C, penalty) selected via 5-fold stratified GridSearchCV across the training set only. Best: C=0.5, L1, CV accuracy 72.1%. L1 regularisation zeroed 14 of 18 coefficients, leaving 4 active features: `sentiment_delta` (+0.666), `lm_sentiment` (−0.484), `risk_persistence` (+0.161), `guidance_score` (+0.152).
...
4. **Alternative models (§10):** Gradient Boosting Classifier (GridSearchCV over n\_estimators, depth, learning rate, subsample) and Ridge/Lasso regression on a continuous return target — both evaluated on the same 18-feature matrix for direct comparability.
...
| **Full-feat LogReg §9a** | **46** | **52.2%** | **+0.151** | **+0.605** | **1.14** | **Selected for §12; 5 active features** |
...
|---|---|---|---|---|---|
| NLP only (18 features, §9a) | 49 | 51.0% | +0.085 | +0.309 | 1.07 |
...
### 3.1 NVIDIA (NVDA) — Revenue Up, Margins Sacrificed, Then Recovered
...
### 3.2 Intel (INTC) — Managed Decline with a Sentiment Spike That Didn't Hold
...
### 3.3 Nike (NKE) — Revenue Lowered Every Quarter; Running Is the One Exception
...
### 3.4 JPMorgan (JPM) — Macro Sensitivity and a Tariff Shock
...
### 5.2 Reactive-vs-Proactive Signal
...
### 5.3 Loughran-McDonald Lexicon — Integration and Comparison
...
### 6.4 Single Prepared Block Per Transcript
...
### 6.5 Feature Selection and Look-Ahead Discipline
...
### 6.6 Guidance Line Features Had Negative ICs
...
### 6.7 n\_wins\_delta Had No Variance
...
### 6.8 Think Tokens (Qwen3 Operational Gotcha)
...
### 6.9 Analyst Ratings Had No Signal
